...
@app.route('/admin')
def admin_page():
    return render_template('admin.html', operators=OPERATORS, assemblies=ASSEMBLIES)

@app.route('/api/admin/stats')
def admin_stats():
    """Full historical stats for admin page."""
    with get_db() as db:
        # Overall totals
        total_picks  = db.execute("SELECT COUNT(*) FROM pick_events").fetchone()[0]
        total_wrong  = db.execute("SELECT COUNT(*) FROM wrong_tray_events").fetchone()[0]
        perfect      = db.execute("SELECT COUNT(*) FROM pick_events WHERE wrong_attempts=0").fetchone()[0]
        avg_ms       = db.execute("SELECT COALESCE(AVG(duration_ms),0) FROM pick_events").fetchone()[0]
        earliest     = db.execute("SELECT MIN(timestamp) FROM pick_events").fetchone()[0]
        latest       = db.execute("SELECT MAX(timestamp) FROM pick_events").fetchone()[0]
        sessions     = db.execute("SELECT COUNT(DISTINCT DATE(timestamp)) FROM pick_events").fetchone()[0]

        # Per operator full history
        op_stats = []
        for op in OPERATORS:
            op_picks = db.execute("""
                SELECT COUNT(*) picks,
                       COALESCE(SUM(wrong_attempts),0) wrong_att,
                       COALESCE(AVG(duration_ms),0) avg_ms,
                       SUM(CASE WHEN wrong_attempts=0 THEN 1 ELSE 0 END) perfect,
                       MIN(timestamp) first_pick,
                       MAX(timestamp) last_pick,
                       COUNT(DISTINCT DATE(timestamp)) active_days
                FROM pick_events WHERE operator_id=?
            """, (op["id"],)).fetchone()
            acc = round(op_picks["perfect"]/max(1,op_picks["picks"])*100,1)

            # Week-by-week accuracy trend (last 4 weeks)
            weekly = db.execute("""
                SELECT strftime('%W',timestamp) wk,
                       COUNT(*) picks,
                       SUM(CASE WHEN wrong_attempts=0 THEN 1 ELSE 0 END) perfect,
                       COALESCE(AVG(duration_ms),0) avg_ms
                FROM pick_events WHERE operator_id=?
                GROUP BY strftime('%W',timestamp) ORDER BY wk DESC LIMIT 4
            """, (op["id"],)).fetchall()
            ...
            op_stats.append({
                ...
                "role":       op["role"],
                "shift":      op["shift"],
                "picks":      op_picks["picks"],
                "wrong_att":  op_picks["wrong_att"],
                "avg_ms":     round(op_picks["avg_ms"]),
                "accuracy":   acc,
                "active_days":op_picks["active_days"],
                "first_pick": op_picks["first_pick"],
                "last_pick":  op_picks["last_pick"],
                "weekly":     [{"week":r["wk"],"picks":r["picks"],
                                "accuracy":round(r["perfect"]/max(1,r["picks"])*100,1),
                                "avg_ms":round(r["avg_ms"])} for r in weekly]
            })

        # Daily activity last 30 days
        daily = db.execute("""
            SELECT DATE(timestamp) d, COUNT(*) picks,
                   COALESCE(SUM(wrong_attempts),0) errors,
                   COUNT(DISTINCT operator_id) ops
            FROM pick_events GROUP BY DATE(timestamp) ORDER BY d DESC LIMIT 30
        """).fetchall()
        ...
        wo_stats = db.execute("""
            SELECT work_order_id,
                   ...
                   COALESCE(SUM(wrong_attempts),0) errors,
                   COALESCE(AVG(duration_ms),0) avg_ms,
                   COUNT(DISTINCT operator_id) unique_ops
            FROM pick_events GROUP BY work_order_id
        """).fetchall()

        # Recent pick log (last 20 across all time)
        recent = db.execute("""
            SELECT * FROM pick_events ORDER BY id DESC LIMIT 20
        """).fetchall()

    overall_acc = round(perfect/max(1,total_picks)*100,1)
    return jsonify({
        "totals": {
            "picks": total_picks, "wrong_attempts": total_wrong,
            "accuracy": overall_acc, "avg_ms": round(avg_ms),
            "sessions": sessions, "data_from": earliest, "data_to": latest,
            "db_path": DB_PATH
        },
        "operators":  op_stats,
        "daily":      [dict(r) for r in daily],
        "work_orders":[{"wo_id":r["work_order_id"],
                        "wo_name":ASSEMBLIES.get(r["work_order_id"],{}).get("name",r["work_order_id"]),
                        "picks":r["picks"],"errors":r["errors"],
